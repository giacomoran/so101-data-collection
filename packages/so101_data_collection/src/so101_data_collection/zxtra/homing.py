...
Usage:
    ...
    python -m so101_data_collection.zxtra.homing

    # Or run the script directly:
    python packages/so101_data_collection/src/so101_data_collection/zxtra/homing.py
...
# Configuration
FPS = 30
LOOP_PERIOD = 1.0 / FPS  # seconds per loop iteration
...
def compute_trajectory_duration(start_pos: float, target_pos: float) -> float:
...
def move_joints_to_target(
    ...
    for joint_name, target_pos in joint_targets.items():
        ...
        if abs(start_pos - target_pos) >= POSITION_TOLERANCE:
            duration = compute_trajectory_duration(start_pos, target_pos)
            if duration > 0.0:
...
def move_to_safe_height(
    ...
    _, current_positions = move_joints_to_target(
        robot, current_positions, joint_targets, time.time(), enable_rerun_logging
    )
