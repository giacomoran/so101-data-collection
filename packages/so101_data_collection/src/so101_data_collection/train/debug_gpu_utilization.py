...
def monitor_gpu(output_file: Path, stop_event: threading.Event, interval: float = 0.5):
...
def compute_stats(csv_file: Path) -> dict:
    ...
    with csv_file.open("r") as f:
        ...
        for row in reader:
            ...
            # Skip first 5 seconds (initialization)
            if elapsed < 5.0:
                continue
...
def main():
    ...
    monitor_thread = threading.Thread(target=monitor_gpu, args=(args.output, stop_event))
