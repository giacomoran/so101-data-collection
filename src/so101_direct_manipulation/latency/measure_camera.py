...
class LatencyStats:
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "failed": self.failed_detections,
            "total_frames": self.total_frames,
            "detection_rate": len(self.measurements) / self.total_frames * 100 if self.total_frames > 0 else 0,
...
def decode_qr(image: np.ndarray, detector: cv2.QRCodeDetector) -> tuple[str | None, np.ndarray | None]:
    """
        ...
        while True:
            ...
            if loop_start - last_log_time >= 2.0:
                ...
                if summary["count"] > 0:
                    ...
                    logger.info(f"[{elapsed:.1f}s] No successful detections yet ({stats.failed_detections} failed)")
