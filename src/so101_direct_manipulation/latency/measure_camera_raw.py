...
from so101_direct_manipulation.shared.setup import (
    CAMERA_HEIGHT,
    CAMERA_WIDTH,
    WRIST_CAMERA_INDEX,
)
...
def setup_camera(
    ...
    logger.info(f"Camera settings: {actual_width}x{actual_height} @ {actual_fps}fps, codec={fourcc_str}")
...
def start_rolling_qr_process(fps: float = 30.0) -> subprocess.Popen:
    ...
    cmd = [
        sys.executable,
        "-m",
        "so101_direct_manipulation.latency.utils_rolling_qr",
        "--fps",
        str(fps),
...
def print_comparison(results: dict[CaptureMethod, Stats]) -> None:
    ...
    print("=" * 80)
    print(f"{'Method':<20} {'Mean':>10} {'Std':>10} {'Min':>10} {'Max':>10} {'Median':>10} {'P95':>10}")
    print("-" * 80)
    ...
    if best_method:
        ...
        if default_summary.get("count", 0) > 0:
            ...
            print(
                f"Improvement over default: {improvement:.1f}ms ({improvement / default_summary['mean_ms'] * 100:.1f}%)"
            )
...
def main():
    ...
    logger.info(f"Camera: {args.camera}, Resolution: {args.width}x{args.height} @ {args.fps}fps")
    ...
    try:
        for method in methods:
            try:
                ...
                if summary["count"] > 0:
                    print(
                        f"mean={summary['mean_ms']:.1f}ms, min={summary['min_ms']:.1f}ms, max={summary['max_ms']:.1f}ms"
                    )
                else:
