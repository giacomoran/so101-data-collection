...
Usage:
    python -m so101_direct_manipulation.latency.measure_inference \
        --policy.path=outputs/cube_direct_manipulation_act_umi_wrist_7_16k/pretrained_model_migrated \
        --dataset_repo_id=giacomoran/cube_direct_manipulation \
        --iterations=1000 \
        --warmup=10
"""
...
def create_synthetic_observation(
    ...
    observation["observation.state"] = np.random.uniform(-1.0, 1.0, size=(state_dim,)).astype(np.float32)
...
def run_inference_chunk(
    ...
    with (
        ...
        observation = prepare_observation_for_inference(observation_frame, device, task=None, robot_type=robot_type)
        observation = preprocessor(observation)

        # Ensure all observations are on the correct device (preprocessor might use config device)
        observation = {k: v.to(device) if isinstance(v, torch.Tensor) else v for k, v in observation.items()}
...
def run_benchmark(
    ...
    n_action_steps = n_action_steps if n_action_steps is not None else policy_n_action_steps
    obs_state_delta_frames = getattr(policy.config, "obs_state_delta_frames", 1)
    use_amp = getattr(policy.config, "use_amp", False)

    logger.info(f"Policy n_action_steps: {policy_n_action_steps}, using: {n_action_steps}")
    ...
    if isinstance(state_feature, dict) and "shape" in state_feature:
        ...
        logger.warning(f"Could not extract state_dim from features, using default: {state_dim}")

    # Get camera names from dataset features
    camera_names = [
        k.replace("observation.images.", "") for k in ds_features.keys() if k.startswith("observation.images.")
    ]
    ...
    if camera_names:
        ...
        if isinstance(cam_feature, dict) and "shape" in cam_feature:
            ...
            logger.warning(f"Could not extract image shape from {first_cam_key}, using default")
