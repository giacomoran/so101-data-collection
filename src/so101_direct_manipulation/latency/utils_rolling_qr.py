...
When used as a module:
    from so101_direct_manipulation.latency.utils_rolling_qr import RollingQRDisplay
...
When used as a script:
    python -m so101_direct_manipulation.latency.utils_rolling_qr
...
class RollingQRDisplay:
    ...
    def _create_window(self) -> None:
        ...
        if not self._window_created:
            ...
            cv2.resizeWindow(self.config.window_name, self.config.qr_size, self.config.qr_size + 60)
    ...
    def is_running(self) -> bool:
        ...
        if self._thread_error is not None:
            return False
        return self._display_thread is not None and self._display_thread.is_alive() and not self._stop_event.is_set()
